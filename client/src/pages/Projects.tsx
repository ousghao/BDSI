...
export default function Projects() {
  ...
  const filters: any = {};
  if (selectedTheme !== "Tous") filters.theme = selectedTheme;
  if (selectedYear !== "Toutes") filters.year = parseInt(selectedYear);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">
              Découvrez les innovations et réalisations de nos étudiants en Big Data et Systèmes Intelligents
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div className="flex flex-col sm:flex-row gap-4 items-center">
              ...
              <Select value={selectedTheme} onValueChange={setSelectedTheme}>
                ...
                <SelectContent>
                  {themes.map((theme) => (
                    <SelectItem key={theme} value={theme} data-testid={`theme-option-${theme.toLowerCase().replace(/[^a-z0-9]/g, '-')}`}>
                      {theme}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              <Select value={selectedYear} onValueChange={setSelectedYear}>
                ...
                <SelectContent>
                  {years.map((year) => (
                    <SelectItem key={year} value={year} data-testid={`year-option-${year.toLowerCase()}`}>
                      {year}
                    </SelectItem>
                  ))}
                </SelectContent>
